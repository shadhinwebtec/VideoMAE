...
def convert_img(img):
    """Converts (H, W, C) numpy.ndarray to (C, H, W) format"""
    if len(img.shape) == 3:
...
class ClipToTensor(object):
    ...
    to a tf.Tensor of shape (m, H, W, C) in the range [0, 1.0]
    """
    ...
    def __call__(self, clip):
        ...
        if isinstance(clip[0], np.ndarray):
            h, w, ch = clip[0].shape
            assert ch == self.channel_nb, 'Got {0} instead of {1} channels'.format(ch, self.channel_nb)
        elif isinstance(clip[0], Image.Image):
            w, h = clip[0].size
        else:
            raise TypeError('Expected numpy.ndarray or PIL.Image but got list of {0}'.format(type(clip[0])))

        np_clip = np.zeros([len(clip), int(h), int(w), self.channel_nb])
        ...
        for img_idx, img in enumerate(clip):
            if isinstance(img, np.ndarray):
                ...
                raise TypeError('Expected numpy.ndarray or PIL.Image but got list of {0}'.format(type(clip[0])))
            img = convert_img(img).transpose(1, 2, 0)  # Convert back to (H, W, C) format
            np_clip[img_idx, :, :, :] = img
        if self.numpy:
            if self.div_255:
...
class ClipToTensor_K(object):
    """Convert a list of m (H x W x C) numpy.ndarrays in the range [0, 255]
    to a tf.Tensor of shape (m, H, W, C) in the range [-1, 1.0]
    """
